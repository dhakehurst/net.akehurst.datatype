...
public class Datatype2HJsonObject extends Object2JsonValue<Object, JsonObject> implements BinaryRule<Object, JsonObject> {
    ...
    private JsonObject getReferenceFor(final Object referedToObject, final BinaryTransformer transformer) {
        final JsonObject reference = new JsonObject();
        reference.add("_referenceTo", referedToObject.getClass().getName());
        for (final Method m : referedToObject.getClass().getMethods()) {
            final Identity idAnn = m.getAnnotation(Identity.class);
            if (null != idAnn) {
                final Object value = Unchecked.supplier(() -> m.invoke(referedToObject)).get();
                ...
                if (null == refAnn) {
                    final JsonValue memberValue = transformer.transformLeft2Right((Class<BinaryRule<Object, JsonValue>>) (Object) Object2JsonValue.class,
                            value);
                    reference.add(this.getMemberName(m), memberValue);
                } else {
                    final JsonObject reference2 = this.getReferenceFor(value, transformer);
                    reference.add(this.getMemberName(m), reference2);
                }
            }
        }
        return reference;
    ...
    public Object constructRight2Left(final JsonObject right, final BinaryTransformer transformer) {
        final String className = right.getString("_class", "<Undefined>"); // should never be undefined due to isValid check
    ...
    public void updateRight2Left(final Object left, final JsonObject right, final BinaryTransformer transformer) {
        for (final Method m : left.getClass().getMethods()) {
            ...
            if (null == queryAnn && null == idAnn && 0 == m.getParameters().length && m.getName().startsWith("get")) {
                ...
                if (null != memberValue) {
                    final Object value = transformer.transformRight2Left((Class<BinaryRule<Object, JsonValue>>) (Object) Object2JsonValue.class, memberValue);
